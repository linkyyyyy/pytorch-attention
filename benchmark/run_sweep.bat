...
echo ========================================================================
echo Ctrl+C to abort, or press any key to start sweep...
pause >nul

echo [SWEEP] corner=%CORNER% engines=%ENGINES% repeats=%REPEATS% duration=%DURATION%s warmup=%WARMUP%s

%PY% run_plan.py --plan %PLAN% --uprof-dir "%UPROF_SESSION_DIR%" --outfile %OUTFILE% --python "%PY%" --uprof-cli %UPROF_CLI% --engines cpu,igpu,npu --duration %DURATION% --warmup %WARMUP% --repeats %REPEATS% --device-id %DEVICE_ID% --cooldown 5
if errorlevel 1 (
